...
async function createAirwallexPaymentIntent(amount: number, currency: string, orderId: string, customerId?: string) {
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/auth/user', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const subscriptionInfo = await storage.getUserSubscriptionWithPlan(userId);
      
      // Get current month AI usage
      const currentDate = new Date();
      const currentMonth = currentDate.getMonth() + 1;
      const currentYear = currentDate.getFullYear();
      const aiUsageTotal = await storage.getUserAiUsageTotal(userId, currentMonth, currentYear);
  ...
  app.post('/api/payment-webhook', async (req, res) => {
    try {
      ...
      if (event_type === 'payment_intent.succeeded') {
        ...
        const transaction = await storage.getPaymentTransaction(paymentIntentId);
        if (!transaction) {
        ...
        const existingSubscription = await storage.getUserSubscription(transaction.userId);
        ...
        if (existingSubscription) {
          ...
          const plan = plans.find(p => {
            const planPrice = transaction.billingCycle === 'monthly' ? p.monthlyPrice :
                            transaction.billingCycle === 'yearly' ? p.yearlyPrice :
                            p.lifetimePrice;
  ...
  app.get('/api/payment-success', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const transaction = await storage.getPaymentTransaction(intent_id as string);
      if (!transaction) {
        return res.status(404).json({ message: "Payment transaction not found" });
      }
  ...
  app.post('/api/ai-usage', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const currentUsage = await storage.getUserAiUsageTotal(userId, month, year);
